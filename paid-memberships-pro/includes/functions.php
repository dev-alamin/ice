...
function pmpro_getLevelCost( &$level, $tags = true, $short = false ) {
	// initial payment
	if ( ! $short ) {
		$r = sprintf( __( 'hello The price for membership is %s now', 'paid-memberships-pro' ), pmpro_formatPrice( $level->initial_payment ) );
	} else {
	...
	if ( (float)$level->billing_amount > 0 ) {
		if ( $level->billing_limit > 1 ) {
			...
			if ( $level->billing_amount === $level->initial_payment ) {
				if ( $level->cycle_number == '1' ) {
					if ( ! $short ) {
						$r = sprintf( __( 'hello The price for membership is %1$s per %2$s.', 'paid-memberships-pro' ), pmpro_formatPrice( $level->initial_payment ), pmpro_translate_billing_period( $level->cycle_period ) );
					} else {
					...
					if ( ! $short ) {
						$r = sprintf( __( 'hello The price for membership is %1$s every %2$d %3$s.', 'paid-memberships-pro' ), pmpro_formatPrice( $level->initial_payment ), $level->cycle_number, pmpro_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
					} else {
...
function pmpro_getLevelsCost( &$levels, $tags = true, $short = false ) {
	...
	// initial payment
	if ( ! $short ) {
		$r = sprintf( __( ' hello The price for membership is %s now', 'paid-memberships-pro' ), pmpro_formatPrice( $initpmt ) );
	} else {
