...
.pmpro_confirmation_wrap {text-align: center;} 
.pmpro_confirmation_wrap p.cnml {font-weight:600; }
</style>
...
<?php
	...
	if(empty($current_user->membership_level))
		$confirmation_message = "<p>" . __('Your payment has been submitted.', 'paid-memberships-pro' ) . "</p>";
	else
...
<?php
	...
	{
		$confirmation_message .= "<p class='cnml'>" . sprintf(__('A welcome email has been sent to %s.', 'paid-memberships-pro' ), $current_user->user_email) . "</p>";
...
<p class="<?php echo pmpro_get_element_class( 'pmpro_actions_nav' ); ?>">
	<?php if ( ! empty( $current_user->membership_level ) ) { ?>
		<?php _e( 'You will be redirected to your account within a few minutes.', 'paid-memberships-pro' ); ?>
	<?php } else { ?>
		<?php _e( 'You will be redirected to your account within a few minutes.', 'paid-memberships-pro' ); ?>
	<?php } ?>
     ...
     <script>
         setTimeout(function(){
            window.location.href = 'https://icepetcare.com/my-pet-information-form/';
         }, 5000);
      </script>    